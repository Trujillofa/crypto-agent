...
class TestCCIBreakoutStrategy:
    ...
    async def test_missing_cci_raises(self, strategy):
        with pytest.raises(ValueError, match="Missing required indicator"):
            await strategy.evaluate("BTCUSDT", {"ema_50": 49000.0, "close_price": 50000.0, "atr_pct": 0.01})

    @pytest.mark.asyncio
    async def test_missing_ema50_raises(self, strategy):
        with pytest.raises(ValueError, match="Missing required indicator"):
            await strategy.evaluate("BTCUSDT", {"cci": 110.0, "close_price": 50000.0, "atr_pct": 0.01})

    @pytest.mark.asyncio
    async def test_missing_atr_pct_raises(self, strategy):
        with pytest.raises(ValueError, match="Missing required indicator"):
            await strategy.evaluate("BTCUSDT", {"cci": 110.0, "ema_50": 49000.0, "close_price": 50000.0})
    ...
    async def test_buy_crossover_above_threshold(self, strategy):
        ...
        await strategy.evaluate("BTCUSDT", {"cci": 90.0, "ema_50": 49000.0, "close_price": 50000.0, "atr_pct": 0.01})

        # Cross above 100 with price > EMA50
        signal = await strategy.evaluate("BTCUSDT", {"cci": 110.0, "ema_50": 49000.0, "close_price": 50000.0, "atr_pct": 0.01})
    ...
    async def test_buy_crossover_exact_threshold(self, strategy):
        ...
        await strategy.evaluate("BTCUSDT", {"cci": 95.0, "ema_50": 49000.0, "close_price": 50000.0, "atr_pct": 0.01})
        signal = await strategy.evaluate("BTCUSDT", {"cci": 100.0, "ema_50": 49000.0, "close_price": 50000.0, "atr_pct": 0.01})
        assert signal.type == SignalType.BUY
    ...
    async def test_buy_blocked_by_trend_gate(self, strategy):
        ...
        await strategy.evaluate("BTCUSDT", {"cci": 90.0, "ema_50": 51000.0, "close_price": 50000.0, "atr_pct": 0.01})

        # Cross above threshold but price < EMA50 (downtrend)
        signal = await strategy.evaluate("BTCUSDT", {"cci": 110.0, "ema_50": 51000.0, "close_price": 50000.0, "atr_pct": 0.01})
    ...
    async def test_sell_not_blocked_by_trend_gate(self, strategy):
        ...
        await strategy.evaluate("BTCUSDT", {"cci": -90.0, "ema_50": 45000.0, "close_price": 50000.0, "atr_pct": 0.01})
        signal = await strategy.evaluate("BTCUSDT", {"cci": -110.0, "ema_50": 45000.0, "close_price": 50000.0, "atr_pct": 0.01})
        assert signal.type == SignalType.SELL

    # 6. Low volatility blocks signal
    @pytest.mark.asyncio
    async def test_buy_blocked_by_low_volatility(self, strategy):
        await strategy.evaluate("BTCUSDT", {"cci": 90.0, "ema_50": 49000.0, "close_price": 50000.0, "atr_pct": 0.001})
        signal = await strategy.evaluate("BTCUSDT", {"cci": 110.0, "ema_50": 49000.0, "close_price": 50000.0, "atr_pct": 0.001})
        assert signal.type == SignalType.HOLD
    ...
    async def test_hold_when_cci_does_not_cross(self, strategy):
        ...
        await strategy.evaluate("BTCUSDT", {"cci": 120.0, "ema_50": 49000.0, "close_price": 50000.0, "atr_pct": 0.01})
        signal = await strategy.evaluate("BTCUSDT", {"cci": 130.0, "ema_50": 49000.0, "close_price": 50000.0, "atr_pct": 0.01})
        assert signal.type == SignalType.HOLD
    ...
    async def test_hold_when_cci_falls_into_overbought(self, strategy):
        ...
        await strategy.evaluate("BTCUSDT", {"cci": 50.0, "ema_50": 49000.0, "close_price": 50000.0, "atr_pct": 0.01})
        ...
        signal = await strategy.evaluate("BTCUSDT", {"cci": 150.0, "ema_50": 49000.0, "close_price": 50000.0, "atr_pct": 0.01})
        assert signal.type == SignalType.BUY
    ...
    async def test_buy_confidence_scales_with_cci_excess(self, strategy):
        ...
        s1 = await strategy.evaluate("BTCUSDT", {"cci": 100.0, "ema_50": 49000.0, "close_price": 50000.0, "atr_pct": 0.01})

        # Re-prime
        strategy2 = CCIBreakoutStrategy({"cci_buy_threshold": 100.0, "cci_sell_threshold": -100.0, "atr_min_pct": 0.005})
        await strategy2.evaluate("BTCUSDT", {"cci": 90.0, "ema_50": 49000.0, "close_price": 50000.0, "atr_pct": 0.01})
        # CCI at 200 → excess = 100 → bonus = min(0.4, 100/200) = 0.4 → confidence = 0.9
        s2 = await strategy2.evaluate("BTCUSDT", {"cci": 200.0, "ema_50": 49000.0, "close_price": 50000.0, "atr_pct": 0.01})
