"""Unit tests for liquidity sweep probe helpers (no DB/network)."""

from __future__ import annotations

from datetime import UTC, datetime, timedelta

from scripts.probe_liquidity_sweep import (
    EventSide,
    HorizonStats,
    LiquidityBar,
    ProbeConfig,
    ...
    detect_failed_downside_breakdowns,
    detect_failed_upside_breakouts,
    evaluate_side,
    forward_passes,
    mae_passes,
    probe_symbol,
...
def _failed_upside_sweep_series() -> list[LiquidityBar]:
    bars = _flat_bars(30)
    sweep_index = 10
    ...
    for index in range(sweep_index + 1, len(bars)):
        close = 99.7 - (index - sweep_index) * 0.3
...
def _failed_downside_sweep_series() -> list[LiquidityBar]:
    bars = _flat_bars(30)
    sweep_index = 12
    bars[sweep_index] = LiquidityBar(
        time=bars[sweep_index].time,
    ...
    for index in range(sweep_index + 1, len(bars)):
        close = 99.3 + (index - sweep_index) * 0.25
        bars[index] = LiquidityBar(
            time=bars[index].time,
            open_price=close - 0.1,
            high_price=close + 0.2,
...
def test_detect_failed_upside_breakouts() -> None:
    bars = _failed_upside_sweep_series()
    events = detect_failed_upside_breakouts(bars, config=_config())
    assert len(events) >= 1
    assert all(event.side is EventSide.SHORT for event in events)
    assert all(event.forward_12h_pct > 0 for event in events)


def test_detect_failed_downside_breakdowns() -> None:
    bars = _failed_downside_sweep_series()
    events = detect_failed_downside_breakdowns(bars, config=_config())
    assert len(events) >= 1
...
def test_evaluate_side_requires_forward_and_mae() -> None:
    ...
    strong = SideResult("BTCUSDT", EventSide.SHORT, events, 2.0, 2.0, 2.0)
    weak_mae = SideResult("BTCUSDT", EventSide.SHORT, events, 0.5, 0.5, 0.5)
