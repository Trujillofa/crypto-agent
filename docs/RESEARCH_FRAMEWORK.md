...
For which mathematical models to implement next (GARCH sizing, regime flags,
meta-labels — overlays on the existing engine, not new signal families), see
...
| Math-model roadmap | `docs/MATH_MODELS_ROADMAP.md` | Implementation order for GARCH / HMM / LightGBM / RL overlays; not a lane brief |
