...
PARAM_GRID = [
    # Wider BB distance (ETH needs more room)
    {
        ...
        "sentiment_gate_threshold": 35.0, "sentiment_panic_threshold": 20.0,
    ...
    # Higher sentiment gate (more selective, fewer but better signals)
    {
        "rsi_oversold": 30.0, "rsi_overbought": 70.0,
        "bb_distance_threshold": 0.010,
        "sentiment_gate_threshold": 45.0, "sentiment_panic_threshold": 20.0,
        "sentiment_boost_threshold": 65.0,
    },
    {
        "rsi_oversold": 30.0, "rsi_overbought": 70.0,
...
def run_backtest(symbol, timeframe, start, end, params, config_path):
    ...
    for line in result.stdout.splitlines():
        if "Total Trades:" in line:
            try: metrics["trades"] = int(line.split(":")[1].strip())
            except: pass
        elif "Win Rate:" in line:
            try: metrics["win_rate"] = float(line.split(":")[1].strip().replace("%","")) / 100
            except: pass
        elif "Sharpe Ratio:" in line:
            try: metrics["sharpe"] = float(line.split(":")[1].strip())
            except: pass
        elif "Profit Factor:" in line:
            try: metrics["profit_factor"] = float(line.split(":")[1].strip())
            except: pass
        elif "Total Return:" in line:
            try: metrics["total_return_pct"] = float(result.stdout.split("Total Return:")[1].split("(")[1].split("%")[0].strip())
            except: pass
...
def run():
    ...
    parser.add_argument("--config", default="/opt/crypto-agent/config/settings.sentiment_macro.yaml")
    ...
    for idx, params in enumerate(PARAM_GRID, 1):
        ...
        for win_idx, (train_start, train_end, test_start, test_end) in enumerate(WFO_WINDOWS, 1):
            print(f"  [{idx}/{total_combos}] params={params['bb_distance_threshold']} "
                  f"[{win_idx}/{len(WFO_WINDOWS)}] {test_start}→{test_end}", flush=True)
            ...
            if metrics and metrics.get("trades", 0) >= args.min_oos_trades:
                ...
                print(f"    → trades={metrics['trades']}, wr={metrics.get('win_rate',0)*100:.1f}%, "
                      f"sharpe={metrics.get('sharpe',0):.2f}")
            else:
                print(f"    → SKIP (trades={metrics.get('trades',0) if metrics else 0} < {args.min_oos_trades})")
        ...
        if oos_sharpe:
            ...
            results.append(row)
            print(f"  >> OOS mean sharpe={row['mean_oos_sharpe']:.2f}, win_rate={row['mean_oos_win_rate']:.1f}%, "
                  f"trades/window={row['mean_oos_trades']:.0f}")
        else:
            print(f"  >> NO VALID OOS WINDOWS")
    ...
    # Write CSV
    fieldnames = [
        "rsi_oversold", "rsi_overbought", "bb_distance_threshold",
        "sentiment_gate_threshold", "sentiment_panic_threshold", "sentiment_boost_threshold",
        "windows_with_trades", "total_oos_trades",
        "mean_oos_sharpe", "mean_oos_win_rate", "mean_oos_trades",
    ]
    ...
    for i, r in enumerate(results[:5], 1):
        print(f"  {i}. RSI({r['rsi_oversold']},{r['rsi_overbought']}) "
