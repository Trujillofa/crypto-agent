#!/usr/bin/env python3
"""Bounded WFO parameter sweep for SOL 4h trend_pullback strategy (Issue #51).

Sweeps entry selectivity × exit models, runs full-period backtest + WFO per
variant, and evaluates results against promotion gates.

Usage: python research/sol_4h_trend_pullback_sweep.py
"""
...
BASE_CONFIG = Path("config/settings.sol_trend_pullback_sparse.yaml")
SYMBOL = "SOLUSDT"
TIMEFRAME = "4h"
...
# Entry selectivity presets (tuned from strategy defaults + paper observations)
ENTRY_MODELS = {
    "conservative": {
    ...
    "sparse": {  # Current paper config baseline
    ...
    "deep_reclaim": {  # Sparse + deep reclaim enabled
        "rsi_reclaim_level": 48,
        "min_trend_strength_pct": 0.008,
        "max_pullback_distance_pct": 0.02,
        "vwap_pullback_distance_pct": 0.03,
        ...
        "deep_reclaim_rsi_level": 48,
        "deep_reclaim_min_macd_hist": -0.01,
        "deep_reclaim_max_ema50_extension_pct": 0.03,
        "deep_reclaim_max_vwap_distance_pct": 0.04,
        "deep_reclaim_arm_window_bars": 4,
        "deep_reclaim_arm_max_ema50_distance_pct": 0.03,
        "deep_reclaim_arm_max_vwap_distance_pct": 0.05,
    },
}
...
def write_temp_config(config: dict) -> str:
    f = tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False, prefix="sol_tp_sweep_")
...
def run_wfo(config_path: str, variant_name: str) -> dict:
    csv_path = f"/tmp/sol_tp_sweep_wfo_{variant_name}.csv"
    cmd = (
...
def main() -> None:
    ...
    print("=" * 80)
    print("SOL 4h trend_pullback WFO Sweep (Issue #51)")
    ...
    print(
        f"Promotion gates: return>{GATES['wfo_return_pct']}%, sharpe>{GATES['wfo_mean_sharpe']}, "
    ...
    for entry_name, entry_params in ENTRY_MODELS.items():
        for exit_name, exit_params in EXIT_MODELS.items():
            variant = f"{entry_name}_{exit_name}"
            ...
            # Approximate WFO return from full-period return (WFO CSV lacks return)
    ...
    print("=" * 120)
    header = (
        f"{'Variant':<28} {'Trades':>6} {'Return%':>8} {'MaxDD%':>7} "
    ...
    for r in all_results:
        ...
        print(
            f"{r['label']:<28} {r['trades']:>6} {r['return_pct']:>8.2f} "
    ...
    # Baseline comparison
    baseline = [r for r in all_results if r["variant"] == "sparse_moderate"]
    if baseline:
        b = baseline[0]
        print(
            f"\nBaseline (current sparse/moderate): "
            f"trades={b['trades']}, return={b['return_pct']:.2f}%, "
    ...
    # Save results
    output_path = Path("research/sol_4h_trend_pullback_sweep_results.json")
